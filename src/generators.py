...
def filter_by_currency(transactions: list[dict], currency: str) -> Iterator:
    ...
    while True:
        trans = list(
            filter(lambda transactions: transactions["operationAmount"]["currency"]["name"] == currency, transactions)
        )
...
def transaction_descriptions(transactions: list[dict]) -> Generator:
    ...
    while True:
        for i in range(len(transactions)):
            ...
            yield transactions[i]["from"]
            yield transactions[i]["to"]
...
for card_number in card_number_generator(1, 99999999):
    print(card_number)
