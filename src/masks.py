...
def get_mask_card_number(card_number: str) -> str:
    ...
    if len(card_number) == 16 and card_number.isdigit():
        start_logger.info(f"Номер карты скрыт: {card_number[:4]} {card_number[4:6]}{"**"} {"****"} {card_number[12:]}")
        return f"{card_number[:4]} {card_number[4:6]}{"**"} {"****"} {card_number[12:]}"
    else:
        start_logger.info("Введен неверный номер карты")
        return str(None)
...
def get_mask_account(account_number: str) -> str:
    ...
    if len(account_number) == 20 and account_number.isdigit():
        ...
        start_logger.info("Введен неверный номер счета")
        return str(None)
